...
router.get('/payments/pending', verifyEmployeeToken, async (req, res) => {
  try {
    // Use strict query with predefined status value to prevent injection
    const PENDING_STATUS = 'pending';
    const payments = await Payment.find({ status: { $eq: PENDING_STATUS } })
    ...
    const sanitizedPayments = payments.map(payment => ({
      ...payment,
      referenceNumber: payment.referenceNumber ? String(payment.referenceNumber).trim() : '',
      customerId: payment.customerId ? {
        fullName: String(payment.customerId.fullName || '').trim(),
        accountNumber: String(payment.customerId.accountNumber || '').trim()
      } : null
    }));    
    ...
    await AuditTrail.log({
      ...
      entityType: 'Payment',
      description: `Employee viewed ${sanitizedPayments.length} pending payments`,
      ipAddress: req.ip || req.connection.remoteAddress,
      userAgent: req.get('User-Agent'),
      severity: 'LOW'
    });
...
router.get('/payments/verified', verifyEmployeeToken, async (req, res) => {
  try {
    ...
    const payments = await Payment.find({ status: { $eq: VERIFIED_STATUS } })
      .populate('customerId', 'fullName accountNumber')
    ...
    const sanitizedPayments = payments.map(payment => ({
      ...
      verifiedBy: payment.verifiedBy ? {
        fullName: String(payment.verifiedBy.fullName || '').trim(),
        employeeId: String(payment.verifiedBy.employeeId || '').trim()
      } : null
    }));
  ...
  try {
    ...
    await AuditTrail.log({
      ...
      payment: {
        ...
        referenceNumber: payment.referenceNumber.toString().trim(),
        status: payment.status,
        rejectionReason: payment.rejectionReason.toString().trim()
      }
    });
  } catch (err) {
